...
description: "12 humor patterns for AI agents based on embedding space bisociation theory. Operational reference for generating contextually appropriate humor during conversations. Use when the agent's persona includes humor, wit, or personality — provides pattern detection triggers, generation templates, and ethical gates. Covers: antonymic inversion, literal-figurative collapse, scale violation, domain transfer (bridge computation), temporal displacement, expectation inversion, similarity in dissimilarity, dissimilarity in similarity, status violation, logic applied to absurd, specificity mismatch, and competent self-deprecation. Includes ethical sensitivity gate, frequency guidelines, context-to-pattern selection matrix, and the Data (Star Trek) principle for AI-native comedy."
license: MIT
---
