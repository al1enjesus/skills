...
export async function getAccessToken(profile, scopes = ['Calendars.Read']) {
  const { cfgPath, cachePath } = profilePaths(profile);
  if (!fs.existsSync(cfgPath)) throw new Error(`Missing profile config: ${cfgPath} (run auth-devicecode.mjs)`);
  ...
  if (!accounts.length) throw new Error(`No accounts in token cache for profile ${profile} (re-run auth-devicecode.mjs)`);
